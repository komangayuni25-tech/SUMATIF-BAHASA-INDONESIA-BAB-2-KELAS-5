...
export function downloadExamQuestionsPDF(questions: Question[]): void {
  ...
  const mapelSlug = CONFIG.MATA_PELAJARAN.replace(/[^a-zA-Z0-9]/g, '_');
  doc.save(`Naskah_Soal_${mapelSlug}_Kelas_${CONFIG.KELAS}_${CONFIG.SEKOLAH.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`);
}
...
export function downloadResultsRecapPDF(results: ExamResult[]): void {
  ...
  const mapelSlug = CONFIG.MATA_PELAJARAN.replace(/[^a-zA-Z0-9]/g, '_');
  doc.save(`Rekap_Nilai_Tes_Sumatif_${mapelSlug}_Kelas_${CONFIG.KELAS}.pdf`);
}
